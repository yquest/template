...
public class StaticHandlerImpl implements StaticHandler {
  ...
  private void sendStatic(RoutingContext context, String path) {
    ...
    // Look in cache
    StaticHandlerImpl.CacheEntry entry = cachingEnabled ? propsCache().get(path) : null;
      ...
      getFileProps(context, sfile, res -> {
        if (res.succeeded()) {
          ...
          if (fprops == null) {
            ...
            if (cachingEnabled) {
              StaticHandlerImpl.CacheEntry now = new StaticHandlerImpl.CacheEntry(fprops, System.currentTimeMillis());
  ...
  private void sendFile(RoutingContext context, String file, FileProps fileProps) {
    ...
    if (rangeSupport) {
      ...
      log.info(file + " passed: " + (end + 1 - (offset == null ? 0 : offset)) + " real: " + fileProps.size());
    }
